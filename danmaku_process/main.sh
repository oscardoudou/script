...
for i in ${!xmls[@]}; do 
    danmaku2ass -o $i.ass -s 1920x1080 -fn "MS PGothic" -fs 48 -a 0.8 -dm 7 -ds 5 ${xmls[i]} ;
    # need shift (add negative delay value to fastword the subtitle so it could sync)
    ffmpeg -itsoffset -16 -i $i.ass -c copy danmaku_$(printf %02d.ass ${i%.ass}) && rm $i.ass ;
done
